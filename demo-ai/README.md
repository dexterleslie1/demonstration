...
从 https://cursor.com/download 下载 deb 安装包
...
>说明：会生成很多unicode符号，导致md格式复制粘贴功能有时候不正常（不能粘贴），目前不使用这个工具。

腾讯元宝 `AI` 聊天窗口虽然支持上传图片，但是不能根据图片使用 `html+css+svg` 生成一模一样的效果，效果差得太远了。

## 阿里通义千问

>说明：支持md格式复制粘贴，目前使用这个查阅资料。
...
>说明：不支持md格式复制粘贴，目前不使用这个工具。

百度文心一言 `AI` 聊天窗口虽然支持上传图片，但是不能根据图片使用 `html+css+svg` 生成一模一样的效果，效果差得太远了。
...
```
1. 不需要你运行或者测试，我会手动测试你的修改
2. 不要修改我的README.md文件
3. 修改代码时，不要修改和此次问题无关的代码
```
