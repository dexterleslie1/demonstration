...
public class Tests {
    ...
    public void test() throws Exception {
        ...
        ObjectResponse<MyVO> response = api.test1(name, "888888");
        Assert.assertEquals("你好，" + name + ",parameter2=injected-p2", response.getData().getField1());
        Assert.assertTrue(response.getData().isField2());

        List<MyPostVO> myPostVOList = new ArrayList<>();
        try {
            api.testPost(myPostVOList, "888888");
        ...
        ObjectResponse<String> response1 = api.testPost(myPostVOList, "888888");
        Assert.assertFalse(response1.getErrorCode() > 0);
        Assert.assertEquals("调用成功,parameter2=injected-p2", response1.getData());
        ...
        response1 = api.testPut("/api/v1/testPut", myPostVOList, "888888");
        Assert.assertFalse(response1.getErrorCode() > 0);
        Assert.assertEquals("调用成功,parameter2=injected-p2", response1.getData());

        ObjectResponse<String> response2 = api.testHeaderWithToken("888888");
        Assert.assertEquals("你提交的token=" + token + ",parameter2=injected-p2", response2.getData());

        String dynamicToken = UUID.randomUUID().toString();
        response2 = api.testHeaderWithDynamicToken(dynamicToken, "888888");
        Assert.assertEquals("你提交的dynamicToken=" + dynamicToken + ",parameter2=injected-p2", response2.getData());
        ...
        ObjectResponse<String> response3 = api.upload(file, "888888");
        Assert.assertEquals("成功上传1个文件,parameter2=injected-p2", response3.getData());
        ...
        response1 = api.delete("1111", "888888");
        Assert.assertEquals("删除成功" + ",parameter2=injected-p2", response1.getData());

        // 测试x-www-form-urlencoded
        String parameter1 = "测试";
        ObjectResponse<String> response6 = api.postWwwFormUrlencoded(parameter1, "888888");
        Assert.assertEquals("提交参数parameter1=" + parameter1 + ",parameter2=injected-p2", response6.getData());

        try {
            api.postWwwFormUrlencoded(null, StringUtils.EMPTY);
            Assert.fail("预期异常没有抛出");
        } catch (BusinessException ex) {
            Assert.assertEquals("没有指定param1参数", ex.getErrorMessage());
        }
