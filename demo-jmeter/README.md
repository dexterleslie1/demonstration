...
## Simple Controller是什么呢？

在 JMeter 中，**Simple Controller（简单控制器）**是最基础的**逻辑控制器**之一，它的作用可以概括为：  

> **把一组请求/元件“打包”成一个逻辑分组，方便组织测试计划结构，但不改变执行逻辑。**
...
- 本身**没有任何条件判断、循环或计时功能**；
- 只是充当一个**容器/文件夹**，把相关的 Sampler、断言、前置/后置处理器等放在一起；
- 在测试计划中起到**结构清晰、层次分明、便于管理**的作用。

可以理解为：  
在 JMeter 的树形结构里，Simple Controller 就像是一个**“虚拟文件夹”**，让你可以把某些元素归到同一组，但运行行为跟把这些元素直接平铺在父节点下基本一样。
...
1. **无执行控制逻辑**
   - 不会像 Loop Controller 那样控制循环次数；
   - 不会像 If Controller 那样根据条件决定是否执行；
   - 不会像 Throughput Controller 那样控制执行比例。
2. **仅影响结构**
   - 对执行顺序：子元素仍然按树中从上到下的顺序执行；
   - 对作用域：子元素的变量、作用域范围与直接放在父节点下相同。
3. **可视化与组织**
   - 在 GUI 中折叠/展开，方便查看复杂测试计划；
   - 给 Simple Controller 起一个有意义的名称（如“用户登录流程”、“订单创建相关请求”），提高可读性。
...
1. **按功能模块分组**
   ```
   线程组
   ├─ Simple Controller（用户注册相关）
   │   ├─ HTTP 请求：检查用户名是否可用
   │   ├─ HTTP 请求：提交注册信息
   │   └─ 响应断言
   ├─ Simple Controller（用户登录相关）
   │   ├─ HTTP 请求：提交登录
   │   └─ 提取 Token
   └─ 查看结果树
   ```

2. **与 Module Controller 配合使用**
   - 在 `common_module.jmx` 中，把“登录流程”放在一个 Simple Controller 下，命名为 `Login_Module`；
   - 主脚本用 **Include Controller** 引入该文件，再用 **Module Controller** 调用 `Login_Module`，实现跨文件模块调用。

3. **临时禁用一批请求**
   - 将一批暂时不需要的 Sampler 放进一个 Simple Controller，然后右键 **禁用（Disable）** 该 Simple Controller，整组请求就不会执行，比一个个禁用更方便。
...
| 控制器                     | 是否控制执行逻辑   | 主要用途                           |
| -------------------------- | ------------------ | ---------------------------------- |
| **Simple Controller**      | 否                 | 仅做结构分组，无逻辑控制           |
| **Loop Controller**        | 是（控制循环次数） | 让子元素重复执行 N 次或一直执行    |
| **If Controller**          | 是（条件判断）     | 满足条件才执行子元素               |
| **Transaction Controller** | 否（但会统计时间） | 把多个请求当成一个事务，统计总耗时 |
| **Module Controller**      | 是（跳转执行）     | 在测试计划中跳转到指定模块执行     |

---

### 五、使用时的注意点

- **不要误以为它能“控制执行”**：很多人第一次用会以为 Simple Controller 有类似“只执行一次”之类的功能，其实没有，需要控制执行逻辑要用 Loop/If/Throughput 等控制器。
- **命名要有意义**：因为它本身不执行任何操作，好的命名能让人一眼看出这一组是做什么的。
- **可以嵌套使用**：Simple Controller 里还可以再放 Simple Controller，形成多层结构，但一般不建议过深，否则结构会难以维护。

---

### 六、示例

详细用法请参考本站示例：https://gitee.com/dexterleslie/demonstration/blob/main/demo-jmeter/demo-simple-controller.jmx

简单总结：  
**Simple Controller 就是一个“无逻辑的容器”，用来整理和分组你的请求，让测试计划更清晰、更易维护。**  
...
## Throughput Controller是什么呢？

在 JMeter 里，**Throughput Controller（吞吐量控制器）**是一个用来**控制其下子元素执行次数/比例**的逻辑控制器，主要用来做**流量分配、压力比例控制**，而不是像普通线程组那样按时间或并发数发压。

下面分点说清楚它是什么、怎么用、注意什么。

---

### 一、它解决什么问题？

- 想让某部分请求**只占总请求量的固定比例**（比如 20% 的流量走 A 接口，80% 走 B 接口）；
- 想让某类请求**在总循环里只执行 N 次**，不随线程数变化而线性放大；
- 在**混合场景**中，需要**精确控制各业务请求的比例或绝对量**，而不是全靠线程数“硬控”。

这些场景用普通线程组 + 定时器都很难做到精准，就可以用 Throughput Controller。
...
- **按“吞吐方式”控制子节点执行**：  
  子节点是**按次执行**还是**按比例执行**，由配置决定。
- 不负责“发多少并发”，而是负责“在已给定的执行机会中，这坨请求能占多少”。
- 常配合：线程组、循环控制器、各种 Sampler 一起使用，实现**复杂压测场景设计**。

---

### 三、关键配置项

在 Throughput Controller 上主要看两个设置：

1. **Throughput（吞吐量值）**
   - 可以填整数（如 `5`）或百分比（如 `30`）。
2. **Per User（是否按用户计算）**
   - 勾选：每个虚拟用户独立按“每用户 N 次/每用户 N%”来算；
   - 不勾选：把整个测试计划/当前作用域当成一个整体，统一分配次数或比例。

常见组合含义如下：

| 模式   | Throughput      | Per User | 含义                                                         |
| ------ | --------------- | -------- | ------------------------------------------------------------ |
| 按次数 | 数字 N（如 10） | 不勾选   | 在整个测试运行期间，该控制器下的请求**总共执行 N 次**        |
| 按次数 | 数字 N（如 10） | 勾选     | 每个虚拟用户各自执行 N 次，总次数 = N × 线程数               |
| 按比例 | 百分比（如 25） | 不勾选   | 在所有可执行的请求中，该控制器占 **25% 的执行机会**          |
| 按比例 | 百分比（如 25） | 勾选     | 每个虚拟用户自己的执行机会中，该控制器占 25%，总体比例仍接近 25% |

> “可执行的请求”指：在它所在作用域内，所有被启用的采样器/控制器，在每次“执行机会”中，根据比例竞争。
...
假设场景：  
线程组 = 10 个用户，循环 100 次，里面有两个 Throughput Controller：A 和 B。

1. **按次数控制**
   - A：Throughput = 50，Per User 不勾选  
     → 整个测试过程中，A 总共执行 **50 次**（不管多少线程）。
   - B：Throughput = 100，Per User 勾选  
     → 每个用户执行 100 次，B 总次数 = 10 × 100 = **1000 次**。

2. **按比例控制**
   - A：Throughput = 30%，Per User 不勾选  
     → 所有请求执行机会中，A 占 **30%**，B 占 70%。
   - A：Throughput = 30%，Per User 勾选  
     → 每个用户自己的执行机会中，A 占 30%，总体上也接近 30%（大样本时更稳）。

---

### 五、使用时的注意点

- **不是“每秒多少个请求”**：Throughput Controller 不直接控制 RPS/QPS，那是 Constant Throughput Timer 等元件做的事。
- **依赖上层调度**：它本身不产生循环或线程，要放在有循环/持续运行的上下文中才有意义。
- **比例模式是“机会分配”**：不是严格数学意义上的“每 10 次里精确 3 次”，而是统计意义上接近目标比例。
- **作用域规则**：和 JMeter 其他控制器一样，子元素受它控制的范围取决于树形结构位置。

---

### 六、和其他控制器/定时器的区别

- **Loop Controller / While Controller**：控制“循环多少次/条件满足就继续”，与比例无关。
- **Constant Throughput Timer**：控制**请求速率（RPS）**，偏“时间维度”；Throughput Controller 偏“执行机会/次数维度”。
- **If Controller**：按条件判断是否执行，不控制比例或次数。

### 七、示例

详细用法请参考本站示例：https://gitee.com/dexterleslie/demonstration/blob/main/demo-jmeter/demo-throughput-controller.jmx

## Include Controller是什么呢？

>提醒：如果要在测试报告中动态显示Include Controller中的标签，需要使用变量方式动态设置，如果一个Include Controller中有多个HTTP Request则导致传递多个标签变量，所以在实际项目中不使用Include Controller，而是情愿复制粘贴重复的逻辑。

在 JMeter 中，**Include Controller（包含控制器）**是一个**用来引入外部测试计划片段**的逻辑控制器，它的作用是：  
**把另一个 `.jmx` 文件里的测试内容，嵌入到当前测试计划中，就像这部分内容原本就写在这里一样**。

---

### 一、它是干什么用的？

- **复用测试片段**：多个测试计划中都会用到同一套登录流程、公共请求、前置准备等，可以把这套流程单独存成一个 `.jmx`，然后通过 Include Controller 引入。
- **拆分大型脚本**：把一个庞大的测试计划拆成若干模块文件，主脚本只保留结构，细节在子文件中维护，便于团队协作和版本管理。
- **场景组合**：根据不同测试需求，在主脚本中灵活组合不同的 Include Controller，加载不同的子脚本，拼出完整的业务流程。

可以理解为：**Include Controller = “导入外部测试片段”的占位符**。

---

### 二、关键配置项

在 Include Controller 界面上只有一个核心字段：

- **Include Path（文件路径）**
  - 填写要引入的外部 `.jmx` 文件的路径；
  - 可以是**绝对路径**（如 `D:/scripts/login_flow.jmx`），也可以是**相对路径**（相对于当前 `.jmx` 所在目录，如 `common/login_flow.jmx`）。

**注意事项：**
- 被引入的文件本身也是一个完整的 JMeter 测试计划（根元素是 `<TestPlan>`）；
- JMeter 只会读取这个文件里的**测试计划内容**，并以子树的形式嵌入到 Include Controller 的位置；
- 路径错误或文件不存在时，运行时会报错或忽略该控制器。
...
1. `login_flow.jmx`（只包含登录相关的 Sampler、断言等）
2. `search_flow.jmx`（只包含搜索相关的 Sampler、断言等）
3. `main_test.jmx`（主测试计划）

在 `main_test.jmx` 中这样组织：

```
线程组
 ├─ Include Controller (Include Path: common/login_flow.jmx)
 ├─ HTTP 请求：打开首页
 ├─ Include Controller (Include Path: common/search_flow.jmx)
 └─ 查看结果树
```

运行 `main_test.jmx` 时，JMeter 会：
- 先把 `login_flow.jmx` 的内容展开到第一个 Include Controller 位置；
- 再执行“打开首页”请求；
- 再把 `search_flow.jmx` 的内容展开到第二个 Include Controller 位置；
- 最终效果和把所有 Sampler 写在同一个文件里是一样的，但结构上实现了**模块化**。

---

### 四、和普通控制器的区别

| 特性     | Include Controller           | Loop Controller / If Controller 等 |
| -------- | ---------------------------- | ---------------------------------- |
| 内容来源 | 外部 `.jmx` 文件             | 当前 `.jmx` 文件内部定义           |
| 作用     | 引入并复用测试片段           | 控制当前文件内的执行逻辑           |
| 灵活性   | 可通过切换引入文件来改变场景 | 只能通过修改当前文件结构调整       |
...
1. **路径管理**
   - 建议使用相对路径，并把公共片段文件和主脚本放在约定的目录结构中，避免因环境不同导致路径失效。
2. **变量作用域**
   - 被引入文件内部的变量、参数等，其作用域遵循 JMeter 正常的变量传递规则；
   - 如果主脚本和被引入文件有同名变量，需要注意覆盖关系。
3. **调试与查看**
   - 在 GUI 模式下，Include Controller 会显示一个小图标，点击可以查看引入内容的概要；
   - 运行时实际展开的是外部文件的内容，所以在查看结果树时看到的是具体 Sampler，而不是 Include Controller 本身。
4. **性能影响**
   - 只是在加载阶段解析外部文件，运行期性能与普通内联脚本基本一致，不会因为“引入”而有额外开销。

---

### 六、适用场景小结

- 多个项目/脚本共用一套**登录、鉴权、初始化**流程；
- 团队多人协作时，每人维护一部分 `.jmx`，最后由主脚本通过 Include Controller 组合；
- 需要在不同测试中**动态选择不同业务模块**进行组合压测。
...
~~详细用法请参考本站示例：https://gitee.com/dexterleslie/demonstration/blob/main/demo-jmeter/demo-module-controller.jmx~~

~~如果你的目标是调用另一个完整的测试计划文件，应该使用**Include Controller**；如果只是想在当前脚本里复用一段逻辑，使用**Module Controller**是最佳实践。~~

## ~~Include Controller和Module Controller组合使用的场景~~

~~JMeter 里，**Include Controller**负责“把外部 `.jmx` 文件引进来”，**Module Controller**负责“在本地测试计划里跳转到某个模块（控制器）执行”。  
两者组合使用的典型场景，就是：**跨文件模块化 + 本地灵活调用**。~~
...
| ~~控制器~~                 | ~~作用范围~~ | ~~关注点~~                                                   |
| -------------------------- | ------------ | ------------------------------------------------------------ |
| ~~**Include Controller**~~ | ~~跨文件~~   | ~~把另一个 `.jmx` 文件的内容嵌入当前测试计划~~               |
| ~~**Module Controller**~~  | ~~本文件内~~ | ~~指定当前测试计划里的某个控制器（如 Loop Controller、Simple Controller）作为“模块”，按需调用~~ |

~~组合起来的本质就是：  
**先用 Include Controller 把外部模块文件“挂进来”，再用 Module Controller 决定什么时候、在什么条件下执行其中的哪个模块。**~~
...
~~**场景**：~~  
- ~~有一个公共模块文件 `common_login.jmx`，里面封装了“登录→获取 Token→保存 Cookie”的流程；~~  
- ~~主脚本 `order_test.jmx` 里有很多业务场景：下单、支付、退款等，都需要先登录。~~

~~**做法**：~~

1. ~~在 `order_test.jmx` 中放一个 **Include Controller**，路径指向 `common_login.jmx`，引入登录模块。~~
2. ~~在 `common_login.jmx` 中，把登录相关 Sampler 包在一个 **Simple Controller** 下，命名为 `Login_Module`。~~
3. ~~在 `order_test.jmx` 的业务流程前，用 **Module Controller** 选择 `Login_Module` 执行。~~

~~**效果**：~~  
- ~~登录逻辑只维护一份（`common_login.jmx`）；~~  
- ~~主脚本通过 Module Controller 在需要的时候调用，不写死在流程中，方便调整顺序或条件。~~

---

#### ~~2. 多环境/多业务线共用基础模块，主脚本做“总控”~~

~~**场景**：~~  
- ~~公司有 A、B 两条业务线，都需要“基础数据准备”“清理环境”等公共操作；~~  
- ~~公共操作写在 `base_module.jmx` 中，里面有多个模块：`InitData`、`CleanEnv` 等。~~

~~**做法**：~~

1. ~~主脚本中通过 **Include Controller** 引入 `base_module.jmx`。~~
2. ~~在 `base_module.jmx` 中，为每类公共操作建一个 **Simple Controller/Transaction Controller**，分别命名。~~
3. ~~主脚本中根据环境变量（如 `${ENV}`）用 **If Controller + Module Controller** 判断：~~
   - ~~若 `ENV=A`，则 Module Controller 调用 `InitData_A` 模块；~~
   - ~~若 `ENV=B`，则调用 `InitData_B` 模块。~~

~~**效果**：~~  
- ~~公共模块统一维护，避免各业务线重复造轮子；~~  
- ~~主脚本通过 Module Controller 做“总控台”，根据环境/参数选择执行哪块逻辑。~~

---

#### ~~3. 动态场景组装：不同测试用例调用不同模块组合~~

~~**场景**：~~  
- ~~有一套通用模块文件 `common_actions.jmx`，里面有：`SearchProduct`、`AddCart`、`CreateOrder`、`PayOrder` 等模块；~~  
- ~~现在想通过一个主脚本，根据 CSV 参数决定本次运行走“搜索+下单”还是“加购+下单+支付”。~~

~~**做法**：~~

1. ~~主脚本中用 **Include Controller** 引入 `common_actions.jmx`。~~
2. ~~在 `common_actions.jmx` 中，每个动作封装成一个 **Simple Controller** 并命名。~~
3. ~~主脚本中读 CSV 数据，根据字段值用 **If Controller + Module Controller** 动态选择：~~
   - ~~若 `scenario=1`：依次调用 `SearchProduct` → `CreateOrder`；~~
   - ~~若 `scenario=2`：依次调用 `AddCart` → `CreateOrder` → `PayOrder`。~~

~~**效果**：~~  
- ~~模块逻辑集中在 `common_actions.jmx` 维护；~~  
- ~~主脚本只负责“读配置 + 决定调用哪些模块”，实现**高度可配置的测试场景**。~~
...
1. ~~**层级关系**~~
   - ~~Include Controller 通常放在主脚本中较上层（如线程组下第一层），负责把外部文件内容拉进来；~~
   - ~~Module Controller 放在具体业务流程中，用来“选中”已经引入进来的某个本地模块。~~

2. ~~**命名与作用域**~~
   - ~~被 Module Controller 调用的模块，必须是**当前测试计划树中真实存在的控制器节点**；~~
   - ~~因此 Include Controller 引入后，相当于把外部文件里的控制器“搬”到了当前树中，Module Controller 才能找到它们。~~

3. ~~**路径与维护**~~
   - ~~Include Controller 的路径建议用相对路径，并与团队约定好模块文件存放目录；~~
   - ~~模块文件尽量保持“功能单一、命名清晰”，方便 Module Controller 调用和维护。~~

4. ~~**与简单复制粘贴的区别**~~
   - ~~直接把公共逻辑复制到每个脚本里，改一次要动很多地方；~~
   - ~~用 Incl~~ude + Module 组合，只需改一处公共文件，所有引用它的主脚本自动生效，更适合**长期维护的大型压测项目**。

## Test Fragment是什么呢？

在 JMeter 中，**Test Fragment（测试片段）** 是一种特殊的测试元件，用于**封装可复用的测试逻辑**，但它**不会自动执行**，必须被其他元件（如 `Module Controller` 或 `Include Controller`）调用才会运行。

---

### 1. 什么是 Test Fragment？

- **定义**：Test Fragment 是 JMeter 测试计划中的一个容器，用于存放一组测试元素（如 Sampler、断言、前置/后置处理器等），这些元素共同完成某个功能模块。
- **特点**：
  - 它**不是线程组的一部分**，因此不会直接执行。
  - 它必须被**显式调用**才能运行。
  - 适合封装**公共逻辑**，例如登录流程、数据准备、接口请求模板等，方便在多个地方复用。

---

### 2. 为什么要用 Test Fragment？

- **提高复用性**：将重复使用的测试逻辑（如用户登录、获取Token）放在 Test Fragment 中，避免重复编写。
- **简化维护**：修改逻辑时只需改一处，所有调用它的地方都会生效。
- **模块化设计**：让测试计划结构更清晰，像编程中的“函数”或“方法”。

---

### 3. 如何创建 Test Fragment？

1. 在测试计划上右键 → **Add** → **Test Fragment** → **Test Fragment**。
2. 在 Test Fragment 下添加需要的测试元素（如 HTTP Request、JSON Extractor 等）。
3. 保存为 `.jmx` 文件后，可以通过 `Include Controller` 引入到其他测试计划中。

---

### 4. 如何调用 Test Fragment？

| 调用方式               | 说明                                                         |
| ---------------------- | ------------------------------------------------------------ |
| **Module Controller**  | 在同一测试计划内，直接引用 Test Fragment 的名称，实现调用。  |
| **Include Controller** | 引入外部的 `.jmx` 文件（该文件中包含 Test Fragment），然后通过 Module Controller 调用引入的片段。 |

---

### 5. 举个例子

假设我们有一个登录流程（发送用户名密码 → 提取 Token），可以将其封装为一个 Test Fragment：

```plaintext
Test Fragment: Login_Fragment
  ├─ HTTP Request (登录接口)
  └─ JSON Extractor (提取 token)
```

然后在主测试计划中，用 `Module Controller` 调用 `Login_Fragment`，这样每次需要登录时，直接执行这个片段即可，无需重复写登录逻辑。

---

✅ **总结**  
Test Fragment 是 JMeter 中用于**封装可复用测试逻辑**的容器，它本身不执行，必须通过 `Module Controller` 或 `Include Controller` 来调用。它的作用类似于代码中的“函数”，能提高测试脚本的**模块化、复用性和可维护性**。
