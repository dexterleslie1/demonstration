...
for ((i=1; i<=$varRedisNodeCount; i++))
do
        grep -q "^sudo -i sh -c \"cd /data/redis-cluster/node${i} && redis-server redis.conf\"" /etc/rc.local || echo "sudo -i sh -c \"cd /data/redis-cluster/node${i} && redis-server redis.conf\"" >> /etc/rc.local
done
