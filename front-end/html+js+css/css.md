...
**CSS `calc()`函数** 是一个强大的CSS计算工具，**允许你在CSS属性值中执行数学运算，动态计算数值**。它最大的价值在于**能够混合使用不同的CSS单位**，实现响应式和灵活的布局。

### 核心作用与价值

#### 1. **混合单位计算**（核心特性）

这是`calc()`最强大的功能，可以在一个表达式中使用不同的CSS单位：
...
```
.wrapper {
  min-height: calc(100vh - 100px); /* 100px是页脚高度 */
}
```
...
1. **运算符前后必须有空格**：`calc(100% - 20px)`✅（正确）
2. 不能写成：`calc(100%-20px)`❌（错误）
3. **支持的操作符**：`+`、`-`、`*`、`/`
4. **可以使用CSS变量**：`calc(var(--spacing) * 2)`
...
.grid {
  ...
  grid-template-columns: repeat(auto-fill, 
    minmax(calc((100% - var(--gap) * (var(--columns) - 1)) / var(--columns)), 
    1fr));
  gap: var(--gap);
}
```
...
```
/* 基础16px，随视口变化，最小18px，最大24px */
h1 {
  font-size: calc(18px + (24 - 18) * ((100vw - 320px) / (1200 - 320)));
}
/* 等同于：clamp(18px, 16px + 2vw, 24px) 的旧浏览器兼容方案 */
```
...
| 特性           | 说明                                     |
| -------------- | ---------------------------------------- |
| **浏览器支持** | 几乎所有现代浏览器都支持（IE9+部分支持） |
| **性能**       | 性能良好，但避免在动画中过度复杂计算     |
| **嵌套**       | 可以嵌套：`calc(calc(100% - 20px) / 2)`  |
| **与CSS变量**  | 完美配合：`calc(var(--size) * 2)`        |

### 与相关技术的对比

| 技术                    | 用途           | 与calc()的关系               |
| ----------------------- | -------------- | ---------------------------- |
| **CSS变量**             | 存储可复用的值 | calc()常与CSS变量结合使用    |
| **min()/max()/clamp()** | 设置值范围     | 可配合calc()使用，功能互补   |
| **媒体查询**            | 响应式断点     | calc()可实现无断点的流畅响应 |
...
4. 
   ...
   **提供回退方案**：
   ...
   .element {
     width: 90%; /* 旧浏览器回退 */
     width: calc(100% - 60px); /* 支持calc的浏览器使用 */
   }
   ```

### 总结

**CSS `calc()`的核心价值是打破了CSS单位之间的壁垒**，让百分比、视口单位、固定像素、em/rem等可以一起计算。它让CSS从"静态声明"向"动态计算"迈进，是实现现代响应式布局的关键工具之一。尤其在结合CSS自定义属性使用时，能创造出极其灵活、可维护的样式系统。
