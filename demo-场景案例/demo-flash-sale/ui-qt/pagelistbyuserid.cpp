    ...
    ui(new Ui::PageListByUserId)
{
    ...
    connect(ui->pushButtonQuery, &QPushButton::clicked, this, [this](){
        ...
        QUrl url;
        if(status.length() == 0) {
            url = QUrl(QString("http://192.168.235.128:8080/api/v1/order/listByUserIdAndWithoutStatus?userId=%1&latestMonth=true").arg(userIdStr));
        } else {
            url = QUrl(QString("http://192.168.235.128:8080/api/v1/order/listByUserIdAndStatus?userId=%1&latestMonth=true&status=%2").arg(userIdStr).arg(status));
        }
                    ...
                    if(errorCode > 0) {
                        ...
                        ui->listWidget->clear();
                         ...
                         for(int i=0;i<data.size();i++) {
                             ...
                             long merchantId = jsonObject["merchantId"].toVariant().toLongLong();
                             ...
                             QString orderStr = QString("订单ID：%1 ，创建时间：%2 ，商家ID：%3 ， 商品列表： %4").arg(id).arg(createTime).arg(merchantId).arg(productList);
