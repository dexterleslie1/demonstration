...
@interface ListByUserIdViewController ()
@property (weak, nonatomic) IBOutlet UIPickerView *pickerView;
@property (weak, nonatomic) IBOutlet UITableView *tableView;

@end

@implementation ListByUserIdViewController
...
- (void)viewDidLoad {
    ...
    // 初始化订单列表 TableView
    // 注册 TableViewCell
    UINib *nib = [UINib nibWithNibName:@"ListByUserIdTableViewCell" bundle:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ListByUserIdTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    if(!cell){
        cell = [[ListByUserIdTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"cell"];
    }
...
- (IBAction)onClickedQuery:(id)sender {
    ...
    NSString *url;
    if([status length] <= 0) {
        url = @"/api/v1/order/listByUserIdAndWithoutStatus";
    } else {
        url = @"/api/v1/order/listByUserIdAndStatus";
    }
