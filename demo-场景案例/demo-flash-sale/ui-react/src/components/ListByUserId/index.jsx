...
export default class ListByUserId extends Component {
  ...
  render() {
    return (
      <div className={styles.container}>
        <div className={styles.title}>用户查询订单列表</div>
        ...
        <div className={styles['order-list-container']}>
          {
            this.state.orderList.length === 0 && (
              <div style={{ color: 'red' }}>没有订单信息</div>
            ) || (
              this.state.orderList.map((item, index) => {
                return <div key={item.id}>
                  <span>订单ID：{item.id}</span>
                  ，<span>创建时间：{item.createTime}</span>
                  ，<span>商家ID：{item.merchantId}</span>
                  ，<span>
                    商品列表：
                    {
