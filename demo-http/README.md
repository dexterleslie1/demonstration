...
>具体用法请参考本站示例：https://gitee.com/dexterleslie/demonstration/tree/main/demo-http/demo-url-encode-n-decode.html

### 一、什么是URL编码？

**URL编码**，也称为 **百分号编码**，是一种将字符转换为可以在互联网上安全传输的格式的机制。

**核心目的**：确保URL中只包含ASCII字符集中允许的、不会引起歧义的字符。它通过将不安全的或非标准的字符转换为一个 `%`符号后跟两个十六进制数字的形式来表示。

------

### 二、为什么需要URL编码？

URL的规范（RFC 3986）定义了一组在URL中具有特殊含义的**保留字符**，以及一些**非保留字符**。

1. **保留字符**：它们在URL中有特定功能，如果用作普通数据，必须进行编码。
   - `:``/``?``#``[``]``@`
   - `!``$``&``'``(``)``*``+``,``;``=`
2. **非保留字符**：这些字符可以安全地直接使用，无需编码。
   - 字母（`A-Z`, `a-z`）
   - 数字（`0-9`）
   - 特殊字符：`-``_``.``~`
3. **需要编码的情况**：
   - **空格**：URL中不能包含空格。通常编码为 `+`（在查询字符串中）或 `%20`。
   - **非ASCII字符**：如中文、日文、表情符号等（例如 `你好`）。这些字符不属于ASCII集，必须被编码。
   - **保留字符用作数据**：如果你想在查询参数的值中传递一个问号 `?`或和号 `&`，它们会被误解为参数的分隔符，因此必须编码。
     - `?`-> `%3F`
     - `&`-> `%26`
   - **不安全字符**：如 `"`、`%`、`<`、`>`、`{`、`}`等，因为它们在传输过程中可能被错误地解释。

**举例说明：**

假设你想搜索 “C++ 教程” 和 “100% 有效”。

- 原始URL可能是：`https://example.com/search?q=C++ 教程 & validity=100% 有效`

- 这个URL问题很大：

  - `空格`是非法的。
  - `+`在查询字符串中有特殊含义（代表空格），但这里它是语言名的一部分。
  - `&`会分隔成两个参数。
  - `%`是编码前缀，直接出现会引起混乱。
  - 中文是非ASCII字符。
...
- **编码后**，浏览器会将其转换为类似这样：
  ...
  `https://example.com/search?q=C%2B%2B%20%E6%95%99%E7%A8%8B&validity=100%25%20%E6%9C%89%E6%95%88`

  - `C++`-> `C%2B%2B`（`+`被编码为 `%2B`）
  - `空格`-> `%20`
  - `教程`-> `%E6%95%99%E7%A8%8B`（UTF-8字节序列的编码）
  - `&`-> `&`（作为分隔符保留，但因为它后面是新的参数名，所以这里不需要编码。但如果它出现在值里，就需要编码为 `%26`）
  - `100%`-> `100%25`（`%`被编码为 `%25`）
...
**URL解码**是URL编码的逆过程。它将编码后的字符串（如 `%20`）转换回其原始的、可读的字符形式（如 `空格`）。

当浏览器或服务器收到一个编码过的URL时，它会自动对其进行解码，以便应用程序能够理解其中的内容。
...
>具体用法请参考本站示例：https://gitee.com/dexterleslie/demonstration/tree/main/demo-http/demo-encode-n-decode-api.html

这是最常见的方式。当你在地址栏输入或JavaScript操作URL时，浏览器会自动处理编码和解码。

你可以在浏览器的开发者工具的 **Console** 标签页中使用JavaScript函数进行测试：

```
// 编码示例
console.log(encodeURIComponent("C++ 教程 & 100%")); 
// 输出： "C%2B%2B%20%E6%95%99%E7%A8%8B%20%26%20100%25"
// encodeURIComponent 会对所有非标准字符进行编码，包括 / ? & 等。

console.log(encodeURI("https://example.com/C++ 教程")); 
// 输出： "https://example.com/C++%20%E6%95%99%E7%A8%8B"
// encodeURI 用于编码整个URL，它不会对 : / ? & = # 等URL结构字符编码。

// 解码示例
console.log(decodeURIComponent("C%2B%2B%20%E6%95%99%E7%A8%8B%20%26%20100%25"));
// 输出： "C++ 教程 & 100%"

console.log(decodeURI("https://example.com/C++%20%E6%95%99%E7%A8%8B"));
// 输出： "https://example.com/C++ 教程"
```

#### 2. 在编程语言中

大多数编程语言都提供了内置库来处理URL编码/解码。
...
- **Python**:
  ...
  # 编码
  encoded_str = urllib.parse.quote("C++ 教程 & 100%")
  print(encoded_str)  # 输出: C%2B%2B%20%E6%95%99%E7%A8%8B%20%26%20100%25
  
  # 解码
  decoded_str = urllib.parse.unquote("C%2B%2B%20%E6%95%99%E7%A8%8B%20%26%20100%25")
  print(decoded_str)  # 输出: C++ 教程 & 100%
  ```
...
- **Java**:
  ...
  // 编码
  String encodedStr = URLEncoder.encode("C++ 教程 & 100%", StandardCharsets.UTF_8);
  System.out.println(encodedStr); // 输出: C%2B%2B+%E6%95%99%E7%A8%8B+%26+100%25
  
  // 解码
  String decodedStr = URLDecoder.decode("C%2B%2B+%E6%95%99%E7%A8%8B+%26+100%25", StandardCharsets.UTF_8);
  System.out.println(decodedStr); // 输出: C++ 教程 & 100%
  ```

  *注意：Java的 `URLEncoder`会将空格编码为 `+`。*

- **PHP**:

  ```
  // 编码
  $encoded_str = urlencode("C++ 教程 & 100%");
  echo $encoded_str; // 输出: C%2B%2B+%E6%95%99%E7%A8%8B+%26+100%25
  
  // 解码
  $decoded_str = urldecode("C%2B%2B+%E6%95%99%E7%A8%8B+%26+100%25");
  echo $decoded_str; // 输出: C++ 教程 & 100%
  ```
...
| 特性         | URL编码                                                      | URL解码                                                      |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **目的**     | 将不安全/非标准字符转换为安全格式，以便通过HTTP传输。        | 将编码后的字符串恢复为原始的可读字符。                       |
| **方法**     | 将字符的UTF-8字节序列表示为 `%XX`的形式。                    | 将 `%XX`序列反向解析为对应的字节，再组合成字符。             |
| **触发场景** | 手动构建URL、表单提交、AJAX请求等。                          | 浏览器接收URL、服务器读取请求参数、程序处理URL时。           |
| **关键函数** | JavaScript: `encodeURIComponent`, Python: `urllib.parse.quote` | JavaScript: `decodeURIComponent`, Python: `urllib.parse.unquote` |

简单来说，**URL编码就是给URL里的“捣蛋字符”戴上面具（变成%XX），让它们能顺利通过安检；而URL解码就是在目的地把面具摘掉，认出它们的真面目。**

## URL和URI的区别

很多人会把URL和URI混为一谈，但其实它们之间是包含与被包含的关系。

简单来说：**URI是“身份证”，用来唯一标识一个资源；而URL是“家庭住址”，告诉你在哪里以及如何找到这个资源。**
...
**URI是一个更广泛的概念，它的唯一作用是“标识”**。你可以把它想象成一个资源的“身份证号码”或“姓名”。只要能唯一确定一个东西，它就是URI。

**URI的核心任务只有一个：命名或标识资源。**

它不关心你如何找到这个资源，也不关心资源在哪里。

**URI有两种主要类型：**

1. **URN (Uniform Resource Name) - 统一资源名称**
   - **作用**：像一个资源的永久、唯一的“名称”或“书号”。
   - **特点**：它不告诉你资源在哪里，只告诉你它叫什么。即使资源从互联网上消失了，它的URN依然有效。
   - **例子**：
     - `urn:isbn:978-0-486-27557-4`(这是《白鲸》这本书的ISBN号)
     - `urn:ietf:rfc:7230`(这是IETF RFC 7230文档的标识符)
2. **URL (Uniform Resource Locator) - 统一资源定位符**
   - **作用**：像一个资源的“地址”或“路线图”。它不仅标识了资源，还指明了如何访问（定位）它。
   - **特点**：它包含了访问协议（如HTTP）、服务器地址和网络位置等信息。
   - **例子**：
     - `https://www.example.com/index.html`
     - `ftp://files.example.com/pub/file.txt`

**URI = URL + URN + 其他可能的标识方式**

------

### 二、URL (Uniform Resource Locator) - 统一资源定位符

**URL是URI的一个子集**。所有的URL都是URI，但并非所有的URI都是URL。

**URL的核心任务是“定位”**。它不仅告诉你资源的名字，还告诉你如何通过特定的协议（如HTTP、FTP）在网络上找到并访问它。
...
- **协议**：访问资源所使用的通信协议。 (`http`, `https`, `ftp`, `mailto`)
- **主机名**：托管资源的服务器的域名或IP地址。 (`www.example.com`)
- **端口**：可选。连接到服务器所使用的端口号。 (`80`, `443`)
- **路径**：资源在服务器上的具体位置。 (`/products/item123`)
- **查询参数**：可选。传递给服务器的额外参数。 (`?category=books&sort=price`)
- **片段标识符**：可选。指向资源内部某个特定部分（如网页中的一个锚点）。 (`#introduction`)

**例子分析：**

```
https://www.example.com:443/products/item?id=123&color=blue#reviews
```

- 它是一个 **URL**，因为它告诉了你如何通过HTTPS协议，连接到`www.example.com`这台服务器的443端口，在`/products/item`路径下，使用`id=123`和`color=blue`这两个参数来获取资源。
- 同时，它也是一个 **URI**，因为它唯一地标识了那个特定的产品页面。

------

### 三、核心区别与关系总结

为了更清晰地理解，我们可以用一个表格和一张图来总结。

| 特性                 | URI (统一资源标识符)                 | URL (统一资源定位符)                                |
| -------------------- | ------------------------------------ | --------------------------------------------------- |
| **核心功能**         | **标识**一个资源                     | **定位**并**访问**一个资源                          |
| **关系**             | **父集/总称**                        | **子集/特例**                                       |
| **关注点**           | “这是什么？” (What is it?)           | “它在哪？怎么去？” (Where is it and how to get it?) |
| **是否包含访问方式** | **不一定**。URN就不包含。            | **一定包含**。必须指明协议和位置。                  |
| **类比**             | **人的身份证号** 或 **一本书的书名** | **一个人的家庭住址**                                |
...
在日常的网络开发和生活中，你几乎总是在使用 **URL**。当我们谈论网页链接、API端点时，我们实际上指的就是URL。

- 当你需要在网络上**找到一个具体的东西并与之交互**（比如打开一个网页、下载一个文件）时，你需要的是一个 **URL**。
- 当你只需要在一个系统内部**唯一地指代一个东西**，而不关心它在哪里时（比如在图书馆数据库中引用一本书），你可以使用 **URN** 或其他形式的URI。

**一个简单的判断方法：**

如果一个标识符告诉你 **“如何去那里”**（包含了协议和服务器地址），那它就是URL，因此它也一定是URI。如果一个标识符只是告诉你 **“那是谁”**（像一个名字或编号），那它可能是一个URN，也是一个URI，但它绝对不是一个URL。