...
/**
 * 主子表 INNER JOIN 演示：子表数据先到，主表数据延迟到达
 * 测试结论：子表数据先到会被存储起来（不会输出到sink表），主表数据后到会inner join先到的子表数据并输出到sink表
 */
public class FlinkSQLParentAndChildTableInnerJoinParentDatumDelayTests {
    public static void main(String[] args) throws Exception {
        ...
        TableResult result = tableEnv.executeSql(
                ...
                "INNER JOIN parent_table parent ON child.dj_id = parent.id"
        );
        result.await();
    }
}
