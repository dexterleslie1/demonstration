...
## 2026/08/05后ZeroSSL签发的证书在MuMu模拟器中访问提示证书不受信任

>情况：因为`ZeroSSL`在上面时间签发的证书使用`Sectigo Public Server Authentication Root R46 x USERTrust RSA CA`新的信任链签发，但是在MuMu模拟器或者旧版本安卓中没有这个新链的根证书，所以在 `fullchain` 里加上由旧根 `USERTrust RSA Certification Authority` 交叉签发的 R46，让旧客户端能走： `叶子 → ZeroSSL CA 2 → (交叉签名) R46 → USERTrust RSA`。

```sh
# 下载交叉签发的R46
curl -fsSL -o r46-cross.crt \
  http://crt.sectigo.com/SectigoPublicServerAuthenticationRootR46_USERTrust.crt

# DER格式转为PEM格式
openssl x509 -in r46-cross.crt -inform DER -out r46-cross.pem

# 合成交叉签发的R46到现有的证书链中
cat fullchain.crt r46-cross.pem > fullchain-compat.crt

# nginx配置
ssl_certificate /usr/local/openresty/nginx/conf/all.all99.cn/fullchain-compat.crt;
ssl_certificate_key /usr/local/openresty/nginx/conf/all.all99.cn/private.key;
```
