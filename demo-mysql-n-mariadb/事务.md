...
脏读（Dirty Read）是指一个事务读取了另一个未提交事务修改过的数据。这种隔离级别问题会对业务层面产生多种负面影响。

### 业务层面的主要影响

#### 1. 数据一致性破坏
- **财务系统**：读取到未确认的转账金额，导致错误的余额计算
- **库存系统**：看到未提交的库存变动，可能引发超卖或误判库存不足
- **订单系统**：读取到未最终确认的订单状态，导致错误处理

#### 2. 决策错误风险
- **报表系统**：基于中间状态数据生成的报表失真
- **数据分析**：统计结果包含未确认交易，影响商业决策
- **风险控制**：基于脏数据做出的风控判断可能失效

#### 3. 用户体验问题
- **电商平台**：用户看到未确认的价格变动后又恢复，产生困惑
- **社交网络**：用户看到未最终发布的评论或内容后又消失
- **游戏系统**：玩家看到未持久化的游戏进度或道具变动

### 典型业务场景示例

#### 银行转账场景
```
事务A: 开始转账(账户A-100，账户B开始余额0+100)
事务B: 在A未提交时读取账户B余额，看到+100并购买了一件价值100的商品
事务A: 因异常回滚
结果: 帐号A没有成功转账给账户B，账户B却余额0也能够购买价值100的商品
```

### 解决方案

1. **提高隔离级别**：使用READ COMMITTED或以上级别
2. **合理设计事务**：
   - 缩短事务执行时间
   - 避免在事务中包含用户交互
3. **应用层校验**：关键操作增加二次确认机制
4. **使用乐观锁**：通过版本号控制并发访问

脏读问题在业务系统中可能导致严重后果，特别是在金融、电商等对数据准确性要求高的领域，必须通过合适的隔离级别和程序设计来避免。



## 事务隔离级别 - 业务异常 - 不可重复读

>提示 `todo`：搜索了资料和 `AI` 问答都没有找到一个实际的业务场景会因为这个原因引起业务异常的。
