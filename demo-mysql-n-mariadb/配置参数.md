...
>注意：
>
>- 在没有配置 slave 时不会自动删除多余日志，此时需要设置 slave_connections_needed_for_purge=0 参数。
>- MariaDB11.4之后才支持此参数，否则启动时报错。
>
...
>启动MariaDB辅助测试：https://gitee.com/dexterleslie/demonstration/tree/main/demo-mysql-n-mariadb/mariadb-server/standalone
...
MariaDB 的 `binlog_format` 参数用于指定二进制日志（Binary Log）的记录格式。这个设置对于数据复制、备份和恢复至关重要。

MariaDB 支持三种 `binlog_format` 格式，它们各有特点：

### Statement (基于语句)

*   **工作原理**：记录实际执行的 SQL 语句。
*   **优点**：日志文件体积较小，因为它只记录语句本身，不记录每一行数据的变化。这可以减少 I/O 开销，提升性能。
*   **缺点**：在执行某些非确定性函数（如 `UUID()`, `NOW()`）时，可能会导致主从数据库之间的数据不一致。
*   **注意**：这是 MariaDB 早期版本的默认格式。

### Row (基于行)

*   **工作原理**：记录每一行数据被更改前后的具体内容。
*   **优点**：数据一致性非常高，能够精确记录每一次数据变更，避免了 Statement 格式可能带来的主从不一致问题。
*   **缺点**：当日志记录大量数据变更（如 `ALTER TABLE` 或批量更新）时，会产生非常庞大的日志文件，占用更多磁盘空间和网络带宽。
*   **注意**：这是 MySQL 8.0 及之后版本的默认格式。

### Mixed (混合模式)

*   **工作原理**：结合了 Statement 和 Row 两种模式。MariaDB 会根据执行的 SQL 语句自动判断，在 Statement 和 Row 之间切换。通常默认使用 Statement，但在遇到可能引起数据不一致的操作时，会自动切换到 Row 模式。
*   **优点**：试图在日志大小和数据一致性之间取得平衡。
*   **缺点**：行为由系统决定，不如 Row 模式那样完全可控。
*   **注意**：**这是 MariaDB 的默认 `binlog_format` 格式**。

### 如何查看和配置

你可以通过以下 SQL 命令查看当前的 `binlog_format` 设置：

```sql
SHOW VARIABLES LIKE 'binlog_format';
```

要永久修改此配置，需要编辑 MariaDB 的配置文件（通常是 `my.cnf` 或 `mariadb-server.cnf`），在 `[mysqld]` 部分添加或修改 `binlog_format` 参数，然后重启 MariaDB 服务使其生效。
...
在生产环境中，为了保证数据的一致性和可靠性，通常**强烈建议将 `binlog_format` 设置为 `ROW`**。
