...
## MariaDB查询指定数据库中各个表记录数

```sql
-- 1. 设置数据库名变量
SET @target_schema = '指定数据库';

-- 2. 清理旧的过程和临时表
DROP PROCEDURE IF EXISTS count_all_tables;
DROP TEMPORARY TABLE IF EXISTS temp_table_counts;

-- 3. 创建存储过程
CREATE PROCEDURE count_all_tables()
BEGIN
    -- 定义变量
    DECLARE done INT DEFAULT FALSE;
    DECLARE t_name VARCHAR(255);
    DECLARE sql_query VARCHAR(1000); -- 增加长度以防表名过长
    DECLARE r_count BIGINT;
    
    -- 定义游标
    DECLARE cur CURSOR FOR 
        SELECT TABLE_NAME, 
               CONCAT('SELECT COUNT(*) INTO @cnt FROM `', TABLE_NAME, '`')
    ...
    -- 创建临时表用于存放结果
    CREATE TEMPORARY TABLE temp_table_counts (
    ...
    -- 循环遍历
    read_loop: LOOP
        FETCH cur INTO t_name, sql_query;
        ...
        -- 将结果插入临时表（而不是直接 SELECT）
        INSERT INTO temp_table_counts (table_name, row_count) VALUES (t_name, r_count);
    ...
    -- 4. 最后一次性输出所有结果
    SELECT table_name AS '表名', row_count AS '精确行数' FROM temp_table_counts ORDER BY row_count DESC;
END;
