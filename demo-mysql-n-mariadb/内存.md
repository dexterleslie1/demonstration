...
```sql
select * from information_schema.processlist where id!=connection_id() and user!='system user'\G;
```

select (@@binlog_cache_size+@@join_buffer_size+@@read_buffer_size+@@read_rnd_buffer_size+@@sort_buffer_size+@@tmp_table_size)/1024/1024;



## 结论

使用`mysql` [实验协助项目](https://gitee.com/dexterleslie/demonstration/blob/master/demo-mysql-n-mariadb/demo-mybatis-plus-assistant) 的`memory`相关测试得出结论如下：
