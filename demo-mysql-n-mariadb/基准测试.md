...
>使用本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/main/demo-mysql-n-mariadb/demo-order-management-app) 协助测试索引 `idx_order_test1` 对批量插入的性能影响。
...
数据库 `innodb buffer pool size` 设置为 `4g`。
...
查看监控：http://192.168.1.17:3000/

测试

```sh
$ wrk -t8 -c16 -d3000000000s --latency --timeout 60 http://192.168.1.185/api/v1/order/initInsertBatch
Running 50000000m test @ http://192.168.1.185/api/v1/order/initInsertBatch
  8 threads and 16 connections
^C  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency   409.54ms  162.21ms   1.68s    69.58%
    Req/Sec     6.15      3.40    20.00     52.39%
  Latency Distribution
     50%  385.91ms
     75%  503.68ms
     90%  628.76ms
     99%  882.64ms
  236747 requests in 100.53m, 59.38MB read
Requests/sec:     39.25
Transfer/sec:     10.08KB
```

订单总数达到 `236764000` 时 `OpenResty Prometheus` 监控

![image-20250829151807146](image-20250829151807146.png)



### 使用分表不分库技术

>分为`32`个表，随着数据量增大，批量插入性能也逐渐下降，最终会保持平稳不再下降（保持平稳时的批量插入性能很差）。
...
Running 50000000m test @ http://192.168.1.185/api/v1/order/initInsertBatch
  8 threads and 16 connections
^C  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency     1.61s     2.19s   11.11s    84.21%
    Req/Sec     5.83      3.76    20.00     40.37%
  Latency Distribution
     50%  538.78ms
     75%    1.59s 
     90%    5.45s 
     99%    8.76s 
  47789 requests in 38.58m, 11.99MB read
Requests/sec:     20.64
Transfer/sec:      5.30KB
```

订单总数达到 `47807000` 时 `OpenResty Prometheus` 监控

![image-20250829133323236](image-20250829133323236.png)



### 使用分库分表技术

>分库分表后，批量插入性能高于以上两种情况，但是数据量达到`3`亿时，批量插入性能开始稍微下降。

使用本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/main/demo-mysql-n-mariadb/demo-order-management-app-sharding-jdbc) 协助测试。

只保留 `common` 模块 `application.properties` 中的 `水平分库分表` 配置。

取消 `inventory.ini` 中 `[sharding-db]`、`[db]` 注释的主机。
...
Running 50000000m test @ http://192.168.1.185/api/v1/order/initInsertBatch
  8 threads and 16 connections
^C  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency   357.75ms  160.36ms   7.60s    94.61%
    Req/Sec     6.46      2.73    10.00     49.96%
  Latency Distribution
     50%  333.44ms
     75%  389.33ms
     90%  463.90ms
     99%  693.23ms
  345135 requests in 126.95m, 86.57MB read
Requests/sec:     45.31
Transfer/sec:     11.64KB
```

订单总数达到 `345151000` 时 `OpenResty Prometheus` 监控

![image-20250830150452338](image-20250830150452338.png)
