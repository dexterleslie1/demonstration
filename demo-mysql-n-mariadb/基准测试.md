...
```sh
ab -n 100000 -c 16 -k http://192.168.1.185/api/v1/order/initInsertBatch
```
...
```sh
# 批量插入结果
$ wrk -t8 -c16 -d30s --latency --timeout 60 http://192.168.1.185/api/v1/order/initInsertBatch
Running 30s test @ http://192.168.1.185/api/v1/order/initInsertBatch
  8 threads and 16 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency   123.30ms   32.70ms 267.96ms   70.12%
    Req/Sec    16.86      5.47    40.00     61.32%
  Latency Distribution
     50%  119.16ms
     75%  143.71ms
     90%  164.45ms
     99%  227.58ms
  3891 requests in 30.04s, 0.98MB read
Requests/sec:    129.53 # 每个批次插入1000条数据，所以批量插入性能为129000条每秒
Transfer/sec:     33.27KB

# 单个事务插入结果
$ wrk -t8 -c2048 -d30s --latency --timeout 60 http://192.168.1.185/api/v1/order/create
Running 30s test @ http://192.168.1.185/api/v1/order/create
  8 threads and 2048 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency   787.52ms  249.40ms   3.90s    89.59%
    Req/Sec   326.24    207.56     1.68k    64.70%
  Latency Distribution
     50%  799.79ms
     75%  830.80ms
     90%  859.73ms
     99%    1.62s 
  77184 requests in 30.10s, 18.70MB read
Requests/sec:   2564.49
Transfer/sec:    636.12KB
```
...
Running 30s test @ http://192.168.1.185/api/v1/order/initInsertBatch
  8 threads and 16 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency   304.57ms  230.02ms   2.00s    88.07%
    Req/Sec     9.50      5.20    30.00     72.27%
  Latency Distribution
     50%  222.47ms
     75%  373.38ms
     90%  565.30ms
     99%    1.16s 
  1735 requests in 30.05s, 445.61KB read
Requests/sec:     57.75
...
Running 30s test @ http://192.168.1.185/api/v1/order/create
  8 threads and 2048 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency   819.24ms  331.26ms   3.67s    79.63%
    Req/Sec   324.45    127.06   818.00     73.20%
  Latency Distribution
     50%  765.71ms
     75%  805.05ms
     90%    1.38s 
     99%    1.70s 
  73871 requests in 30.09s, 17.89MB read
Requests/sec:   2454.66
Transfer/sec:    608.87KB
```
...
- `t_order` 表数据量`1`亿并且有二级索引时，批量插入性能为没有二级索引的`50%`。
- `t_order` 表数据量`1`亿并且没有二级索引时，批量插入性能没有下降。
- `t_order` 表数据量`1`亿并且有二级索引时，单个事务插入性能稍微低于无二级索引情况。
