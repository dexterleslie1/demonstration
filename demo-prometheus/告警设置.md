...
> 参考：https://blog.csdn.net/Allenyongyu/article/details/154658028

步骤如下：

1. 下载并登录桌面版钉钉，下载地址：https://www.dingtalk.com/

2. 设置群机器人：点击需要运维接收通知的群设置 > 机器人，添加自定义WebHook机器人

3. alertmanager.yml设置使用dingtalk发送告警通知
   ...
   receivers:
   # 钉钉告警接收器：通过 prometheus-webhook-dingtalk 转发到钉钉机器人
   - name: 'dingtalk'
     webhook_configs:
     - url: 'http://prometheus-webhook-dingtalk:8060/dingtalk/default/send'
       # 告警恢复时是否同样发送通知。true 表示当告警从 firing 变为 resolved 时，会再发一条“已恢复”的通知到钉钉，便于确认问题已解决；设为 false 则仅发送触发告警，不发送恢复通知。
       send_resolved: false
   ```

4. Docker Compose中运行dingtalk webhook用于对接钉钉接口

   ```yaml
     # 钉钉告警中转：接收 Alertmanager webhook 并转发到钉钉机器人
     prometheus-webhook-dingtalk:
       image: timonwong/prometheus-webhook-dingtalk:v2.1.0
       restart: always
       volumes:
         - /etc/localtime:/etc/localtime:ro
         - ./dingtalk-webhook.yml:/etc/prometheus-webhook-dingtalk/config.yml:ro
       command:
   ...
   复制上面钉钉机器人的WebHook和加签填写到dingtalk-webhook.yml中

   ```yaml
   # prometheus-webhook-dingtalk 配置文件
   # 钉钉机器人 Webhook 地址请在钉钉群 -> 群设置 -> 智能群助手 -> 添加机器人 -> 自定义 中获取
   # 安全设置可选用：自定义关键词（建议包含「告警」）、加签（需填写下方 secret）
   timeout: 5s
   
   targets:
     default:
       # 替换为你的钉钉机器人 Webhook 地址，格式：https://oapi.dingtalk.com/robot/send?access_token=YOUR_ACCESS_TOKEN
       url: https://oapi.dingtalk.com/robot/send?access_token=YOUR_ACCESS_TOKEN
       # 若机器人开启了加签，在此填写 SEC开头的 secret，未开启则留空或删除此行
