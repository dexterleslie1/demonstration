...
> 全局异常处理详细的例子请参考 [链接](https://github.com/dexterleslie1/demonstration/tree/master/demo-spring-boot/demo-spring-boot-global-exception-handle)

使用`@ControllerAdvice + @ExceptionHandler`注解声明处理的异常逻辑
...
`application.properties`加入如下设置才能在`404`情况下抛出`NoHandlerFoundException`
...
`Application`中通过`@Import(value = GlobalExceptionHandler.class)`引入全局异常处理机制
