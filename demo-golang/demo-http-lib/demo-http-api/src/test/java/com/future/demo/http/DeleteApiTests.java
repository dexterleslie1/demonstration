...
public class DeleteApiTests {
    ...
    DeleteApiFeign deleteApiFeign;

    @Before
    public void before() {
        deleteApiFeign = Feign.builder()
                ...
                .target(DeleteApiFeign.class, "http://localhost:" + localServerPort);
    }

    @Test
    public void test() {
        String responseStr = this.deleteApiFeign.testDeleteSubmitParamByUrl1("v1");
        Assert.assertEquals("提交参数param1=v1", responseStr);

        responseStr = this.deleteApiFeign.testDeleteSubmitParamByUrl2("v1");
        Assert.assertEquals("提交参数param1=v1", responseStr);

        FormDataDTO formDataDTO = new FormDataDTO();
        formDataDTO.setParam1("v1");
        responseStr = this.deleteApiFeign.testDeleteSubmitParamByJSON(formDataDTO);
        Assert.assertEquals("提交参数param1=v1", responseStr);
    }
}
