...
参考
https://juejin.cn/s/linux%20quick%20cpu%20benchmark

### sysbench

centOS8 安装 sysbench，参考
...
- 什么是顺序和随机读写？
  ...
  https://blog.csdn.net/markzy/article/details/135869536
...
- 是否可以通过工具判断当前运行软件是顺序读写还是随机读写呢？
  ...
  https://unix.stackexchange.com/questions/279963/is-there-a-way-to-monitor-disk-i-o-patterns-i-e-random-or-sequential-i-o
...
- 什么是同步和异步 I/O 呢？
  ...
  /etc/fstab 配置 defaults 默认使用异步（async） I/O 模式，NOTE：一般操作系统挂载卷默认使用 async 模式。

  https://en.wikipedia.org/wiki/Fstab

  

  通过命令 mount -v 查看被挂载卷的 I/O 模式，如果不显示 async 和 sync 表示卷使用默认 async 模式，如果显示 sync 则表示卷使用 sync 模式。
