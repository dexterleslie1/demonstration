...
## Seata各个版本和SpringBoot各个版本的兼容性

Seata 与 Spring Boot 的版本兼容性主要取决于 **Seata Spring Boot Starter** 的版本。Seata 的版本迭代非常快，不同大版本（如 1.x 和 2.x）对 Spring Boot 的依赖和兼容性有显著差异。

为了确保你的项目（当前时间 2026 年）稳定运行，建议优先使用 **Seata 2.x** 系列，因为 Seata 1.x 已经停止维护，且对新版本 Spring Boot 支持较差。

以下是详细的版本兼容对照表和建议：

### 1. 推荐组合（2026年主流选择）

对于当前时间（2026年），绝大多数项目应该已经升级到了 **Seata 2.x** 以配合较新的 Spring Boot 版本。

| Seata 版本               | 推荐 Spring Boot 版本 | 说明                                                         |
| :----------------------- | :-------------------- | :----------------------------------------------------------- |
| **Seata 2.0.x - 2.2.x**  | **3.0.x - 3.4.x**     | **首选推荐**。完全支持 Jakarta EE (Servlet 6.0+)，适配 Spring Boot 3 生态。 |
| **Seata 1.8.x - 1.11.x** | **2.7.x - 2.10.x**    | **旧项目维护**。仅支持 Java EE (javax.servlet)，无法运行在 Spring Boot 3+ 上。 |

### 2. 详细兼容性说明

#### Seata 2.x (Current - 适配 Spring Boot 3)
这是目前（2026年）的主流选择，解决了 Spring Boot 3 的 `javax` 到 `jakarta` 的包名迁移问题。

*   **Seata 2.0.0+**
    *   **Spring Boot**: 3.0.x, 3.1.x
    *   **Java**: 17+
    *   **特性**: 完全重写了 Starter，支持自动配置（AutoConfiguration），去除了对旧版 Spring Data 的强依赖。
*   **Seata 2.1.0+ (推荐)**
    *   **Spring Boot**: 3.1.x, 3.2.x
    *   **特性**: 增强了对 Nacos 2.3+ 和 Redis 7+ 的注册/配置中心支持，修复了 2.0 的部分序列化 Bug。
*   **Seata 2.2.0+ (最新稳定)**
    *   **Spring Boot**: 3.2.x, 3.3.x (视具体发布而定)
    *   **特性**: 优化了 TCC 模式的性能，支持更灵活的多数据源配置。

#### Seata 1.x (Legacy - 仅限 Spring Boot 2)
如果你还在维护旧项目，且无法升级 Spring Boot 3，请使用此分支。**注意：该分支已停止更新，存在安全风险。**

*   **Seata 1.11.0 (Final)**
    *   **Spring Boot**: 2.7.18 (最后一个支持 Java 8 的主流版本)
    *   **Java**: 8 - 17
    *   **限制**: 不支持 Spring Boot 3，因为依赖的 `javax.servlet` 在 Spring Boot 3 中已被移除。
*   **Seata 1.7.x / 1.8.x**
    *   **Spring Boot**: 2.6.x, 2.7.x
    *   **注意**: 1.8.x 开始对 Spring Cloud 2021+ 有更好的支持。

### 3. 依赖配置示例 (Maven)

#### Spring Boot 3 + Seata 2.2.x (最新推荐)
```xml
<properties>
    <spring-boot.version>3.2.10</spring-boot.version> <!-- 或 3.3.x -->
...
<dependencies>
    ...
    <!-- 如果使用 Nacos 作为注册/配置中心 -->
    <dependency>
        <groupId>com.alibaba.cloud</groupId>
        <artifactId>spring-cloud-starter-alibaba-nacos-discovery</artifactId>
        <version>2023.0.0.0</version> <!-- 适配 Spring Boot 3 -->
    </dependency>
</dependencies>
```

#### Spring Boot 2 + Seata 1.11.x (旧项目)
```xml
<properties>
    <spring-boot.version>2.7.18</spring-boot.version>
    <seata.version>1.11.0</seata.version>
</properties>
...
<dependencies>
    ...
    <!-- Seata 1.x 的 groupId 是 com.alibaba -->
    <dependency>
...
1.  **`javax` vs `jakarta` 错误**：
    *   **现象**：`ClassNotFoundException: javax.servlet.ServletContext`。
    *   **原因**：在 Spring Boot 3 项目中使用了 Seata 1.x。
    *   **解决**：必须升级到 Seata 2.x。

2.  **Spring Cloud 版本冲突**：
    *   Seata 2.x 通常需要配合 **Spring Cloud 2022+** (如 `2022.0.4`, `2023.0.0`) 使用。
    *   如果你使用的是 Spring Cloud Alibaba，注意其最新版本（2022.0.0.0+）才支持 Spring Boot 3。

3.  **配置文件变更**：
    *   Seata 2.x 中，配置前缀从 `spring.cloud.alibaba.seata` 变更为 `spring.cloud.seata` 或直接 `seata.*`（取决于是否使用 Spring Cloud 模式）。请参考官方文档更新 `application.yml`。

### 5. 总结建议

鉴于当前时间是 **2026年**：
*   **新项目/重构项目**：请毫不犹豫选择 **Seata 2.2.x** 配合 **Spring Boot 3.2+**。
*   **老项目升级**：如果还在使用 Spring Boot 2.7，请尽快规划升级到 Spring Boot 3 + Seata 2.x，因为 Spring Boot 2.7 的商业支持和社区维护在 2023 年底就已经结束了，继续使用存在极大的安全漏洞风险。
...
各个微服务 `application.properties` 的 `Seata XA` 模式配置
...
>提示：
>
>- `todo` 没有业务需求，暂时不研究此模式。
>- 适宜使用的业务场景：1、长事务业务。2、需要和外部封闭系统集成为完整的业务事务场景。
...
>详细用法请参考本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/master/spring-cloud/demo-spring-cloud-alibaba-seata-eureka)
...
使用本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/master/spring-cloud/demo-spring-cloud-alibaba-seata-xa) 协助测试
...
```sh
curl -X GET http://192.168.1.185/order/createPerfTest
```

执行性能测试

```sh
$ wrk -t8 -c2048 -d30s --latency --timeout 60 http://192.168.1.185/order/createPerfTest
Running 30s test @ http://192.168.1.185/order/createPerfTest
  8 threads and 2048 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency     3.44s   633.21ms   4.93s    89.85%
    Req/Sec   120.28    112.15   656.00     76.86%
  Latency Distribution
     50%    3.61s 
     75%    3.66s 
     90%    3.73s 
     99%    4.42s 
  16702 requests in 30.10s, 4.14MB read
Requests/sec:    554.89
Transfer/sec:    140.86KB
```
...
使用本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/master/spring-cloud/demo-spring-cloud-alibaba-seata-at) 协助测试
...
Running 30s test @ http://192.168.1.185/order/createPerfTest
  8 threads and 2048 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency     5.27s     1.39s   13.50s    85.84%
    Req/Sec    65.63     66.91   535.00     86.47%
  Latency Distribution
     50%    5.65s 
     75%    5.80s 
     90%    5.92s 
     99%    8.64s 
  10566 requests in 30.10s, 2.62MB read
Requests/sec:    351.07
Transfer/sec:     89.02KB
```
...
使用本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/master/spring-cloud/demo-spring-cloud-alibaba-seata-tcc) 协助测试
...
Running 30s test @ http://192.168.1.185/order/createPerfTest
  8 threads and 2048 connections
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency     4.50s     1.09s    7.88s    85.21%
    Req/Sec    65.04     63.69   590.00     88.79%
  Latency Distribution
     50%    4.81s 
     75%    4.96s 
     90%    5.19s 
     99%    5.58s 
  12521 requests in 30.09s, 3.11MB read
Requests/sec:    416.05
...
在未调优的情况下性能有高到低：`XA` 模式 > `TCC` 模式 > `AT` 模式。

## 分布式事务 - Seata启用Prometheus监控

>提示：没有找到开源的Seata Server Grafana Dashboard。

参考官方文档：https://seata.apache.org/zh-cn/docs/user/apm/prometheus/

具体用法请参考本站示例：https://gitee.com/dexterleslie/demonstration/tree/main/demo-prometheus/demo-docker-compose-prometheus

application-seata.yml中配置启用Prometheus
...
seata:
  metrics:
    ...
    exporter-prometheus-port: 9080
```

seata-server开放9080端口
...
```yaml
scrape_configs:
  - job_name: 'Seata Server Metrics'
    scrape_interval: 15s
    static_configs:
    - targets: ['192.168.1.181:9080']
      labels:
        instance: seata-server
```

访问http://localhost:9080/mertrics查看Seata监控指标。
