...
void print_peers(struct peer peers[], int peer_len)
{
    printf("Peer list:\n");
    for (int i = 0; i < peer_len; i++)
    {
        printf("\t%s:%u\n", inet_ntoa(peers[i].addr), ntohs(peers[i].port));
    }
}
 ...
 * 广播peer数组
 */
void broadcast_peers(int fd, struct sockaddr_in cli, struct peer peers[], int peer_len)
{
    ...
    struct peer peer_current = peers[1];
    cli.sin_addr = peer_current.addr;
    cli.sin_port = peer_current.port;
    if (sendto(fd, &peers[0], sizeof(peers[0]), 0, (struct sockaddr *)&cli, cli_len) < 0)
    {
        perror("sendto");
    }
}
...
int main(void)
{
    ...
    for (;;)
    {
         ...
         */
        printf("Received peer [%s:%u] %s\n", inet_ntoa(cli.sin_addr), ntohs(cli.sin_port), buf);

        // 记录p2p节点
        peers[peer_len].addr = cli.sin_addr;
        peers[peer_len].port = cli.sin_port;
        peer_len++;
        print_peers(peers, peer_len);

        broadcast_peers(fd, cli, peers, peer_len);
