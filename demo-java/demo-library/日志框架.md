...
<configuration scan="true" scanPeriod="30 seconds">
    ...
    <appender name="FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
        ...
        <async class="ch.qos.logback.classic.AsyncAppender">
            <!-- 后台线程名称前缀 -->
            <threadNamePrefix>Logback-Async-</threadNamePrefix>
            <!-- 队列大小（默认 256，调大减少阻塞） -->
            <queueSize>1024</queueSize>
            <!-- 队列满时丢弃的最低级别（默认 INFO，设为 DEBUG 避免丢弃关键日志） -->
            <discardThreshold>DEBUG</discardThreshold>
            <!-- 是否忽略异常（避免日志写入失败导致应用崩溃） -->
            <ignoreExceptions>true</ignoreExceptions>
            <!-- 引用实际输出的 FILE Appender -->
            <appender-ref ref="FILE"/>
        </async>
    </appender>

    <!-- 根 Logger（控制所有 Logger 的级别） -->
    <root level="INFO">
        <!-- 同时输出到控制台和文件（生产环境可移除 CONSOLE） -->
        <appender-ref ref="CONSOLE"/>
        <appender-ref ref="FILE"/>
    </root>

    <!-- 可选：自定义 Logger（如针对某个包单独设置级别） -->
...
1. **MDC 上下文传递**：  
   异步线程（如 `@Async`、线程池）需手动传递 MDC 上下文，可通过 `TaskDecorator` 实现：
   ```java
   @Bean
...
2. **避免日志阻塞**：  
   ...
   - 禁止在日志中执行耗时操作（如 `toString()` 复杂对象、数据库查询）。  
   - 使用 `isDebugEnabled()` 等条件判断避免不必要的日志计算：
     ```java
     if (logger.isDebugEnabled()) {
         logger.debug("Complex object: {}", expensiveToString(obj));
     }
     ```
   
3. **监控日志性能**：  
   通过 Micrometer 或 APM 工具（如 SkyWalking）监控日志写入耗时，及时发现瓶颈。

4. **日志文件分离**：  
   按业务模块分离日志文件（如 `order.log`、`user.log`），避免单一文件过大影响性能。

**六、验证与测试**

配置完成后，通过以下方式验证效果：
- **压测工具**：使用 JMeter 或 Gatling 模拟高并发请求，观察应用 CPU、内存、磁盘 IO 是否稳定。  
- **日志完整性**：检查滚动后的日志文件是否包含完整上下文（如 `traceId`）。  
- **异步验证**：通过日志线程名确认是否为 `Logback-Async-` 前缀的后台线程。  


通过以上配置，可显著提升高并发场景下 Logback 的性能和可靠性，确保日志系统不会成为应用的瓶颈。



#### 异步日志的配置

在 Spring Boot 中使用 Logback 实现异步日志是解决高并发场景下日志阻塞业务线程、提升应用吞吐量的核心手段。以下从**原理、配置方式、关键参数、MDC 传递**到**完整示例**进行详细说明，并提供生产环境的最佳实践。

**一、异步日志的核心原理**

Logback 的异步日志通过**生产者-消费者模型**实现：  
- **生产者**：业务线程将日志事件（`ILoggingEvent`）放入一个**无锁队列**（如 `ArrayBlockingQueue`）。  
- **消费者**：后台专用线程从队列中取出日志事件，执行实际的 IO 写入操作（如写入文件、控制台）。  

这种设计将耗时的 IO 操作从业务线程剥离，避免了业务线程因等待日志写入完成而被阻塞，显著提升高并发场景下的性能。

**二、两种异步日志配置方式**

Logback 提供了两种异步日志实现方式，分别适用于不同场景：

**方式 1：通过 `AsyncAppender` 包装同步 Appender（传统方式）**

通过 `AsyncAppender` 包裹一个实际的同步 Appender（如 `FileAppender`、`ConsoleAppender`），将日志事件先存入队列，再由后台线程写入目标输出。  
**适用场景**：需要兼容旧版本 Logback（<1.1.7）或需要对特定 Appender 单独控制异步行为。

**方式 2：使用 `AsyncLogger`（推荐，Logback 1.1.7+）**

Logback 1.1.7 引入了 `AsyncLogger`，直接实现异步日志记录接口（`AsyncAppenderBase`），内部使用更高效的无锁队列（`ConcurrentLinkedQueue`）和批量写入策略，性能优于 `AsyncAppender`。  
**适用场景**：新版本 Logback（≥1.1.7），追求更高性能的异步日志。

**三、关键配置参数详解**

无论是 `AsyncAppender` 还是 `AsyncLogger`，核心参数均围绕**队列管理、日志级别控制、MDC 传递**展开：

| 参数/组件           | 说明                                                         | 推荐值/注意事项                                              |
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `queueSize`         | 异步队列容量（仅 `AsyncAppender`）                           | 默认 256；高并发建议 512~2048（根据内存调整，队列越大，内存占用越高但阻塞概率越低） |
| `includeCallerData` | 是否包含调用者数据（如类名、方法名、行号）                   | 默认 `false`（计算调用者数据耗时，高并发建议关闭）           |
| `discardThreshold`  | 队列满时丢弃的最低日志级别（仅 `AsyncAppender`）             | 默认 `INFO`；建议设为 `DEBUG`，避免丢弃关键日志（如 `WARN`、`ERROR`） |
| `asyncLogger`       | 是否启用 `AsyncLogger`（在 `logback-spring.xml` 根节点配置） | 推荐设为 `true`（`<configuration asyncLogger="true">`）      |
| `maxFlushTime`      | 后台线程刷新队列的最大时间（毫秒，仅 `AsyncLogger`）         | 默认 1000ms；设为 `0` 表示立即刷新（不推荐，可能丢失未写入的日志） |
| `MDC` 传递          | 异步线程是否继承父线程的 MDC 上下文                          | 需手动处理（见下文“MDC 上下文传递”）                         |

**四、完整配置示例**

**示例 1：基于 `AsyncAppender` 的传统配置**

适用于需要兼容旧版本或对特定 Appender 单独控制异步的场景：
...
<configuration scan="true" scanPeriod="30 seconds">
    ...
    <appender name="CONSOLE" class="ch.qos.logback.core.ConsoleAppender">
        ...
        <filter class="ch.qos.logback.classic.filter.ThresholdFilter">
            <level>INFO</level> <!-- 生产环境控制台仅输出 INFO 及以上 -->
        </filter>
    </appender>

    <!-- 滚动文件输出（核心） -->
    <appender name="FILE_SYNC" class="ch.qos.logback.core.rolling.RollingFileAppender">
        <file>${LOG_DIR}/app-sync.log</file>
        <rollingPolicy class="ch.qos.logback.core.rolling.TimeBasedRollingPolicy">
            <fileNamePattern>${LOG_DIR}/app-sync-%d{yyyy-MM-dd}.log</fileNamePattern>
            <maxHistory>30</maxHistory> <!-- 保留 30 天历史日志 -->
        </rollingPolicy>
        <encoder>
            <pattern>%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n</pattern>
            <charset>UTF-8</charset>
    ...
    <appender name="FILE_ASYNC" class="ch.qos.logback.classic.AsyncAppender">
        <appender-ref ref="FILE_SYNC"/> <!-- 包装的同步 Appender -->
        <queueSize>1024</queueSize> <!-- 队列容量调大 -->
        <discardThreshold>DEBUG</discardThreshold> <!-- 队列满时丢弃 DEBUG 及以下 -->
        <includeCallerData>false</includeCallerData> <!-- 关闭调用者数据 -->
        <ignoreExceptions>true</ignoreExceptions> <!-- 忽略日志写入异常 -->
    </appender>

    <!-- 根 Logger：同时输出到控制台和异步文件 -->
    <root level="INFO">
        <appender-ref ref="CONSOLE"/>
        <appender-ref ref="FILE_ASYNC"/> <!-- 使用异步文件 Appender -->
    </root>
</configuration>
```

**示例 2：基于 `AsyncLogger` 的高性能配置（推荐）**

Logback 1.1.7+ 推荐使用 `AsyncLogger`，性能更优（无锁队列+批量写入）：
...
<configuration 
	...
	asyncLogger="true" <!-- 启用 AsyncLogger -->
	asyncLoggerBaseBuffered="true"
	asyncLoggerBufferSize="4096"
	asyncLoggerWaitStrategy="Block"> <!-- 等待策略（可选：Block/Sleep/Yield） -->

    <!-- 日志存储目录 -->
    <property name="LOG_DIR" value="logs"/>

    <!-- 异步文件输出（AsyncLogger 直接写入） -->
    <appender name="FILE_ASYNC" class="ch.qos.logback.core.rolling.RollingFileAppender">
        <file>${LOG_DIR}/app-async.log</file>
        <rollingPolicy class="ch.qos.logback.core.rolling.SizeAndTimeBasedRollingPolicy">
            <!-- 按时间+大小滚动：每天一个文件，单个最大 100MB -->
            <fileNamePattern>${LOG_DIR}/app-async-%d{yyyy-MM-dd}-%i.log</fileNamePattern>
            <maxFileSize>100MB</maxFileSize>
            <maxHistory>30</maxHistory>
            <totalSizeCap>10GB</totalSizeCap> <!-- 总日志不超过 10GB -->
        </rollingPolicy>
        <encoder>
            <!-- 简化日志格式（减少 CPU 消耗） -->
    ...
    <logger name="ROOT" level="INFO" additivity="false">
        <appender-ref ref="FILE_ASYNC"/> <!-- 异步写入文件 -->
        <!-- 生产环境可关闭控制台，或限制级别 -->
        <!-- <appender-ref ref="CONSOLE"/> -->
    </logger>

    <!-- 可选：自定义包级 Logger（如业务包单独设置级别） -->
    <logger name="com.example.service" level="INFO" additivity="false">
...
异步线程默认不会继承父线程的 MDC 上下文（如请求 ID、链路追踪 ID），需手动处理。以下是两种解决方案：

**方案 1：在线程池中使用 `TaskDecorator`（推荐）**

通过 Spring 的 `TaskDecorator` 复制父线程的 MDC 上下文到异步线程：
...
**方案 2：使用 `AsyncAppender` 的 `includeCallerData`（不推荐）**

`AsyncAppender` 支持通过 `includeCallerData="true"` 自动捕获调用者信息，但会**增加日志写入耗时**（需计算调用者类名、方法名、行号），高并发场景不建议使用。

**六、生产环境最佳实践**

1. **优先使用 `AsyncLogger`**：Logback 1.1.7+ 的 `AsyncLogger` 性能优于 `AsyncAppender`（无锁队列+批量写入），推荐升级 Logback 版本并启用 `asyncLogger="true"`。  
2. **控制队列大小**：`queueSize` 不宜过大（建议 ≤2048），避免内存溢出；队列过小会导致频繁阻塞，需根据业务并发量压测调整。  
3. **避免日志阻塞**：  
   - 禁止在日志中执行耗时操作（如 `toString()` 复杂对象、数据库查询）。  
   - 使用 `isDebugEnabled()` 条件判断减少不必要的日志计算：  
     ```java
     if (logger.isDebugEnabled()) {
         logger.debug("Complex object: {}", expensiveToString(obj)); // 仅 DEBUG 级别时计算
     }
     ```
4. **监控日志性能**：通过 APM 工具（如 SkyWalking、Pinpoint）监控日志写入耗时，观察异步队列是否经常满（`AsyncAppender` 的 `discardedCount` 指标）。  
5. **分离关键日志**：将 `ERROR` 级别日志单独输出到另一个异步 Appender，避免与 `INFO` 日志竞争队列资源。  
6. **关闭不必要的日志**：生产环境关闭 `DEBUG` 和 `TRACE` 级别日志，仅保留 `INFO` 及以上必要日志。  

**七、验证配置是否生效**

1. **查看日志线程名**：异步日志的线程名通常为 `AsyncAppender-AsyncLogger-XX`（`AsyncAppender`）或 `AsyncLogger-XX`（`AsyncLogger`），而非业务线程名（如 `http-nio-8080-exec-XX`）。  
2. **压测验证**：使用 JMeter 模拟高并发请求（如 1000QPS），观察应用 CPU、内存、磁盘 IO 是否稳定，日志是否无延迟或丢失。  
3. **检查 MDC 上下文**：在日志中打印 `%X{traceId}`，验证异步线程是否能正确传递请求 ID（如通过 `curl` 发送带 `traceId` 的请求，检查日志是否一致）。  


通过以上配置和优化，Logback 异步日志能有效解决高并发场景下的性能瓶颈，确保日志系统成为应用的“助力”而非“拖累”。
...
- 使用本站 [示例](https://gitee.com/dexterleslie/demonstration/tree/main/demo-java/demo-library/demo-logging-slf4j-parent/demo-spring-boot-logback) 协助测试，发现配置了异步日志和同步日志的性能没有明显区别（性能差不多）。异步日志配置如下：
  ...
  ```sh
  wrk -t8 -c4096 -d30s --latency --timeout 30 http://192.168.1.185/api/v1/testPerfWithDebugLog
  ```

  

- 每个请求都打印日志的性能低于每个请求都不打印日志的性能，前者的性能损耗为后者的 `50%`。

- 高并发接口不应该打印大量的日志，因为性能会损耗很严重。不是高并发接口可以打印业务所需要的日志。
