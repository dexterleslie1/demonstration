...
- 右键使用上面步骤的`class`选择上下文菜单中的`Use Selected Objects`功能后弹出`New Object Set`面板，在面板中选择`References`>`Merged incoming references`视图后点击`OK`按钮。
- 此时跳转到`References`视图，选择`Aggregate by field`协助查找出对象被引用类中的具体字段。
