...
在 Shell 命令中，单引号（`'`）和双引号（`"`）的主要区别在于**是否允许变量扩展和特殊字符转义**。

### 单引号（' '）
- **完全原样输出**：单引号内的所有字符都会被视为普通字符
- **不进行变量替换**
- **不进行命令替换**
- **不支持转义字符**
...
name="张三"
echo "Hello $name, today is $(date)"
# 输出：Hello 张三, today is Mon Dec 25 10:30:00 CST 2023
```
